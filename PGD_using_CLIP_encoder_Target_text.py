...
# Load x and y images
x_original= load_image("test_img/img_dog.jpg")
x=x_original.clone().detach().requires_grad_(True)  # Perturbable image
y = tokenizer(["a big horn goat"], padding=True, return_tensors="pt")
...
with torch.no_grad():
    y_features = model.get_text_features(**y).detach()
...
# Original Image
Image.open("test_img/img_dog.jpg").show()
...
perturbed_image.save("test_img/perturbed_dog.jpg", dpi=(300, 300))
...
with torch.no_grad():
    ...
    final_cosine_similarity = torch.nn.functional.cosine_similarity(x_features_final, y_features)
    ...
    # y_features_final = y_features_final / y_features.norm(p=2, dim=-1, keepdim=True)

    # Calculate cosine similarity
    orig_cosine_similarity = torch.nn.functional.cosine_similarity(x_features_original, y_features)
